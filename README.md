...
In MVVM, you abstract your code to include a viewModel, 
which is a file that holds the values to be presented in your view. 

The View Controller owns the viewModel which owns the Model and in turn, 

the Model updates the View Model, which is in turn, used to update the View Controller, which updates the view.

Codable Protocol. A type that can convert itself into and out of an external representation.
It is used by the type can be both encoded as well as decoded.
